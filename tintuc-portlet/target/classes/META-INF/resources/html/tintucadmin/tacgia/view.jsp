...
<%
	...
	String name = "";int status = 0; int total = 0;
	if(Validator.isNotNull(ParamUtil.getString(request, "tentacgia"))){
		name = ParamUtil.getString(request, "tentacgia");
	}
	//int status = ParamUtil.getInteger(request, "sudung");
	
	//System.out.print(status);
	PortletURL pagingUrl = renderResponse.createRenderURL();
	pagingUrl.setParameter("name", name);
	pagingUrl.setParameter("sudung", String.valueOf(status) );
	pagingUrl.setParameter("jspPage", "/html/tintucadmin/tacgia/view.jsp");
	pagingUrl.setParameter("tabs", tabTacGia);

	PortletURL searchURL = renderResponse.createRenderURL();
	searchURL.setParameter("tabs", tabTacGia);
	...
	List<News_NguonTin> listObj = TinTucAdminUtil.listTacGia(name, status, start, end); 
	total = TinTucAdminUtil.listTacGia(name, status, 0, 0).size();
	searchContainer.setTotal(total);
	searchContainer.setResults(listObj);
%>
<portlet:renderURL var="addURL">
	<portlet:param name="jspPage" value="/html/tintucadmin/tacgia/add.jsp" />
	<portlet:param name="tabs" value="<%=tabTacGia %>" />
</portlet:renderURL>

<portlet:actionURL var="deleteTacGiaURL" name="deleteTacGia">
	<portlet:param name="jspPage" value="/html/tintucadmin/tacgia/view.jsp" />
	<portlet:param name="tabs" value="<%=tabTacGia %>" />
</portlet:actionURL>

<portlet:renderURL var="searchTacGiaURL">
	<portlet:param name="jspPage" value="/html/tintucadmin/tacgia/view.jsp" />
	<portlet:param name="tabs" value="<%=tabTacGia %>" />
</portlet:renderURL>
...
<form  action="" method="post" id="frmAction">
	<h3 class="row-fluid header smaller lighter blue" style="position: relative;overflow: hidden;">
		<span style="width: 55%!important; display: block;float: left;">Quản lý danh sách tác giả - nguồn tin</span>
...
<!-- 			<th style="text-align: center; width:10%">Sắp xếp</th> 
	...
	<tr>
	    <portlet:renderURL  var="editURL">
	      ...
	      <portlet:param name="jspPage" value="/html/tintucadmin/tacgia/edit.jsp"/>
	      <portlet:param name="tabs" value="<%=tabTacGia %>" />
	    </portlet:renderURL>
...
function confirmDelete(){
	...
	if(count>0){
		if(confirm("Bạn chắc chắn muốn xóa những đối tượng này?")){
			$("#frmAction").attr('action','<%=deleteTacGiaURL%>');
			$("#frmAction").submit();
		}
	}else{
