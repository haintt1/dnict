...
<div class="row-fluid header smaller lighter blue">
	<h4>Thống kê bài viết theo kiểu tin</h4>			
</div>
<%
...
<form  action="" method="post" id="frmthongke" name="frmthongke">
	<table class="table table-bordered">
		...
		<tr>
			<td class="key" valign="top" style="text-align: center;" colspan="6">
				...
				<a class="btn-small btn btn-success" href="#" id="btnReportKieuTin" >Xuất Excel</a>
			</td>
		</tr>
	</table>
	<table class="table table-bordered table-list" id="resultsTable">
		<tr style="text-align: center;">
			...
			<th class="key" style="width: 40%;">Kiểu tin</th>
		...
		<%	
			...
			int stt=0;			
			if(listKieuTin.length>0){
				for (int i=0;i<listKieuTin.length;i++) {
					stt=stt+1;	
					List<News_Article> objNews = TinTucAdminUtil.listTintucKieuTin(dateBegin, dateEnd, status, i);
		%>
		<tr>
			...
			<td style="text-align: center; width: 200px;">
				<portlet:renderURL  var="danhsachchitietURL">
					<portlet:param name="jspPage" value="/html/thongketintuc/kieutin/chitiet.jsp" />
					<portlet:param name="tabs" value="<%=tabtkkieutin %>"/>
					<portlet:param name="kieutin" value="<%=String.valueOf(i)%>" />
		...
		<tr><th></th><th>Tổng số bài viết: </th><th style="text-align: center;"><%=listnewtotal.size() %></th><th></th></tr>	
		<%} else{%>
	...
	$j("#btnReportKieuTin").click(function(){
		$j("#resultsTable").table2excel({
			name: "ThongKeSLKieuTin",
			filename: "ThongKeSLKieuTin",
			fileext: ".xls",
			exclude_img: true,
			exclude_links: true,
			exclude_inputs: true
