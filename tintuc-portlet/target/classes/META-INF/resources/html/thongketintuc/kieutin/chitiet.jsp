...
<%
String tabtkkieutin = ThongKeTinTucField.value_tabkieutin;
...
String status="-1";
int kieutin = 0; 
if(Validator.isNotNull(ParamUtil.getString(request, "kieutin"))){
	if(Validator.isNumber(ParamUtil.getString(request, "kieutin"))){
		kieutin = ParamUtil.getInteger(request, "kieutin");
	}
}
...
pagingUrl.setParameter("jspPage", "/html/thongketintuc/kieutin/chitiet.jsp");
pagingUrl.setParameter("tabs", tabtkkieutin);

PortletURL searchURL = renderResponse.createRenderURL();
searchURL.setParameter("jspPage", "/html/thongketintuc/kieutin/chitiet.jsp");
searchURL.setParameter("kieutin",String.valueOf(kieutin));
pagingUrl.setParameter("tabs", tabtkkieutin);
...
List<News_Article> listArticle = ThongKeTinTucUtil.thongKeSoLuongByKieuTin(Integer.valueOf(kieutin), Integer.valueOf(status) , dateBegin, dateEnd, 0, 0);
total = listArticle.size();
searchContainer.setTotal(total);
searchContainer.setResults(listArticle);
String[] listKieuTin = {"Tin ảnh", "Tin văn bản", "Tin video"};
%>
...
<h4 style="margin-top:10px">Thống kê bài viết của 1 kiểu tin</h4>
<form  action="<%=searchURL.toString()%>" method="post" id="frmthongke" name="frmthongke">
	<table class="table table-bordered">
		...
		<tr>					
			<td class="key" valign="top" style="text-align: center;" colspan="6">
				...
				<a class="btn btn-success btn-small" href="#" id="btnReportthongketheoKieuTin">Xuất Excel</a>
				<portlet:renderURL var="cancelURL">
					<portlet:param name="tabs" value="<%= tabtkkieutin %>"/>
					<portlet:param name="jspPage" value="/html/thongketintuc/kieutin/list.jsp"/>
				</portlet:renderURL>
	...
	<table class="table table-bordered table-list" id="resultsTable">
		...
		<tr>
			<th colspan="6">
				<span style="text-transform: uppercase;">TỔNG CỘNG <span style="color: #cc1314"><%=total%></span> BÀI VIẾT THUỘC KIỂU TIN <span style="color: #cc1314"><%=listKieuTin[kieutin] %></span></span>
			</th>
		</tr>
		<%
		int stt =1;
...
$j(document).ready(function(){
	...
	$j("#btnReportthongketheoKieuTin").click(function(){
		$j("#resultsTable").table2excel({
			name: "ThongKeChiTietKieuTin",
			filename: "ThongKeChiTietKieuTin",
			fileext: ".xls",
			exclude_img: true,
			exclude_links: true,
			exclude_inputs: true
