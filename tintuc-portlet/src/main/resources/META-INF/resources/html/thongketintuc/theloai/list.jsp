...
<div class="row-fluid header smaller lighter blue">
	<h4>Thống kê bài viết theo thể loại</h4>			
</div>
<%
	...
	//List<News_Article> listType = ThongKeTinTucUtil.thongKeSoLuongByTheLoai(dateBegin, dateEnd, status, typeid);
...
<form  action="" method="post" id="frmthongke" name="frmthongke">
	<table class="table table-bordered">
		...
		<tr>
			<td class="key" valign="top" style="text-align: center;" colspan="6">
				...
				<a class="btn-small btn btn-success" href="#" id="btnReportSLTheLoai" >Xuất Excel</a>
			</td>
		</tr>
	</table>
	<table class="table table-bordered table-list" id="resultsTable">
		<tr style="text-align: center;">
			...
			<th class="key" style="width: 40%;">Thể loại</th>
		...
		<%
			List<News_Type> objType = TinTucAdminUtil.searchNew_type("", 0, 0, 0);
			int stt=0;			
			if(objType.size()>0){
		...
		<tr>
			...
			<td><%=objType.get(i).getName() %></td>
			
			<td style="text-align: center;">
				<%
					List<News_Article> listart = ThongKeTinTucUtil.thongKeChiTietByTheLoai(dateBegin, dateEnd, status, objType.get(i).getId());						
				%>
				<%=listart.size()%>
			</td>	
			<td style="text-align: center; width: 200px;">
				<portlet:renderURL  var="danhsachchitietURL">
					<portlet:param name="jspPage" value="/html/thongketintuc/theloai/chitiet.jsp" />
					<portlet:param name="tabs" value="<%=tabtktheloai %>"/>
					<portlet:param name="typeid" value="<%=String.valueOf(objType.get(i).getId()) %>" />
	...
	$j("#btnReportSLTheLoai").click(function(){
		$j("#resultsTable").table2excel({
			name: "ThongKeTheLoai",
			filename: "ThongKeTheLoai",
			fileext: ".xls",
			exclude_img: true,
			exclude_links: true,
			exclude_inputs: true
