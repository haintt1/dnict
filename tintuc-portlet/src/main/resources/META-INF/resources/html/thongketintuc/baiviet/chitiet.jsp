...
pagingUrl.setParameter("jspPage", "/html/thongketintuc/baiviet/chitiet.jsp");
pagingUrl.setParameter("tabs", tabtkbaiviet);

PortletURL searchURL = renderResponse.createRenderURL();
searchURL.setParameter("jspPage", "/html/thongketintuc/baiviet/chitiet.jsp");
pagingUrl.setParameter("tabs", tabtkbaiviet);
...
List<News_Article> listArticle = ThongKeTinTucUtil.thongKeChiTiet(Long.valueOf(theloai), userId ,Integer.valueOf(status) , dateBegin, dateEnd, 0, 0);
total = ThongKeTinTucUtil.thongKeChiTiet(Long.valueOf(theloai), userId ,Integer.valueOf(status) , dateBegin, dateEnd, 0, 0).size();
...
<h4 style="margin-top:10px">Thống kê bài viết của 1 tài khoản</h4>
<form  action="<%=searchURL.toString()%>" method="post" id="frmthongke" name="frmthongke">
	<table class="table table-bordered">
		<tr>
			<th class="key label2">Người đăng tin:</th>
			<td valign="top">
				<%
				List<User> user1 = PhanQuyenAdminUtil.getUsersByID(userId);				
				List<News_Article> listart = ThongKeTinTucUtil.thongKeChiTietByUserId(userId);	
				for(int s =0; s < user1.size(); s++){
				%>
					<%=user1.get(s).getFullName()%>				
			</td>
			<%}	%>
		...
		<tr>
			...
			<td class="key" valign="top" style="text-align: center;" colspan="2">
	...
	<table class="table table-bordered table-list" id="resultsTable">
		...
		<tr>
			<th colspan="6">
				<span style="text-transform: uppercase;">TỔNG CỘNG <span style="color: #cc1314"><%=total%></span> BÀI VIẾT</span>
			</th>
		</tr>
		<%
		int stt =1;
