...
<title>Chỉnh sửa tin PDF</title>
</head>
<body>
<%
String tabDangtin = TinTucAdminField.value_tabdangtin;
String idnewspfd = ParamUtil.getString(request, "id");
News_Article objNewsPDF = (News_Article) News_ArticleLocalServiceUtil.fetchNews_Article(Long.valueOf(idnewspfd));
...
tenbai = HtmlUtil.unescape(objNewsPDF.getTitle());

boolean checkYeucauHieuChinh = false;
boolean checkCategoryOfUser  = false;
if(objNewsPDF.getContent() != null){
		noidung  = objNewsPDF.getContent();
		...
		checkYeucauHieuChinh = TinTucAdminUtil.checkYeuCauHieuChinhByUserIdAnd(themeDisplay.getUserId(), objNewsPDF.getId());
		checkCategoryOfUser  = TinTucAdminUtil.checkCategoryOfUser(themeDisplay.getUserId(), objNewsPDF.getId());
		objectActivity 		 = TinTucAdminUtil.getNewAtivityById(themeDisplay.getUserId(), objNewsPDF.getId());
}
%>
...
<portlet:actionURL name="updateNewsArticlePDF" var="updateNewsArticlePDFURL">
</portlet:actionURL>
...
<portlet:renderURL var="checkchuyenmucURL" windowState="<%=LiferayWindowState.POP_UP.toString()%>">
	<portlet:param name="id" value="<%= String.valueOf(objNewsPDF.getId()) %>" />
				...
				<div aria-multiselectable="true" class="panel-group" role="tablist">
					<fieldset aria-labelledby="Title" class=" " role="group">
						<div aria-labelledby="Header" class="in  " id="Content" role="presentation">
							...
							<aui:form name="frmAddNewsPDF" action="<%=updateNewsArticlePDFURL.toString() %>"method="post" enctype="multipart/form-data" autocomplete="off">
								<%
								if(objNewsPDF.getStatus() == 5){
								%>
								...
								<%if(objectActivity!= null){ %>
									...
									<%} %>
								<%}else { %>
								<h3 class="title-pm">Đăng tin, bài viết pdf</h3>
								<%} %>
								<table class="tbcoreadmin" style="width: 100%">
									<tr>
										<td colspan="100">
											<label class="control-label">Tiêu đề</label>
											<aui:input type="hidden" name="id" value="<%=objNewsPDF.getId() %>"/>
									...
									<tr>
										<%
											String check = "";
											String check_flipbook = "";
											if(objNewsPDF.getLoaitintuc() == 1 ){
												check = "checked";
											}
											if(objNewsPDF.getLoaitintuc() == 3){
									...
									<tr>
										<td colspan="100">
											...
											<aui:input name="urlsmallimage" value="<%=objNewsPDF.getUrlimagesmall() %>" label="" type="input" cssClass="linkimage">	
											</aui:input>
										</td>	
									</tr>
									<tr>
										<td colspan="100">
											...
											<aui:input name="tomtat" value="<%=objNewsPDF.getInfo() %>" label="" type="textarea" cssClass="">	
											</aui:input>
										</td>	
									</tr>
									...
									<tr>
										<td colspan="100" style="padding-bottom: 20px;">
												...
												<span class="listdanhmuc">
													...
													List<News_Article2Category> listart2Cat = TinTucAdminUtil.getListArticle2CategoryByArticleId(objNewsPDF.getId());
									...
									<tr>
										...
										List<News_AttachFile> listDinhkem = DocumentUpload.getLinkdinhkembyIdAndObject(objNewsPDF.getId(), 9);
										String  idpdf = "";
										String urlDomain="";
										...
										<portlet:actionURL name="deleteFilePDF" var="deleteFilePDFURL">
											...
											<portlet:param name="idNews" value="<%=String.valueOf(objNewsPDF.getId()) %>"/>
										</portlet:actionURL>
										<tr>
											<td colspan="100">
												...
												<aui:input name="idnews" type="hidden" value="<%=objNewsPDF.getId()%>"></aui:input>
												<aui:input name="flag" type="hidden" value="<%=request.getAttribute("flag")%>"></aui:input>
												<a class="filedinhkem" href="<%= urlDomain + listDinhkem.get(i).getUrl()%>" target="_blank"><%=listDinhkem.get(i).getTen()%></a> 
												- <a id="btndelete" href="javascript:void(0)" att-url="<%=deleteFilePDFURL %>" class="fa fa-delete"><img style="width: 14px;" alt="" src="<%= request.getContextPath()%>/images/delete.ico"/></a>
							...
							<%}%>
									...
									<tr>
										<td style="padding-top: 10px">
											...
											<input name="<portlet:namespace/>tuKhoa" value="<%=objNewsPDF.getTukhoa() %>" type="text" id="tags" class="form-control"/>
										</td>
									</tr>
									<tr>
										<td>
								     		...
								     		<%
								     			...
								     			if(objType.size() > 0){
								     				for(int s=0; s < objType.size(); s++){
								     					String selected = "";
								     					if(objNewsPDF.getTypeid() == objType.get(s).getId()){
								     						selected = "selected";
								     					}
								     		%>
									...
									<tr>
										...
										<div class="twocolumn1">											
											...
											<input name='<portlet:namespace/>ngayxuatban' id="ngayxuatban" value="<%=df.format(objNewsPDF.getNgayxuatban())  %>"  type="text" class="form-control"></input>	
										</div>
										<div class="twocolumn2">					
											<label class="control-label">Ngày kết thúc</label>
											<input name='<portlet:namespace/>ngayketthuc' id="ngayketthuc" 
										       value="<%= (objNewsPDF.getNgayketthuc() != null) ? df.format(objNewsPDF.getNgayketthuc()) : "" %>" 
									...
									<tr>
									     		...
									     		List<News_NguonTin> objTacGia = TinTucAdminUtil.listTacGia("", 0, 0, 0); 
									     			if(objTacGia.size() > 0){
									     				for(int s=0; s < objTacGia.size(); s++){
									     					String selected = "";
									     					if(objNewsPDF.getNguontin() == objTacGia.get(s).getId()){
									     						selected = "selected";
									     					}
									     		%>
										...
										<div class="twocolumn2">
											...
											<input name='<portlet:namespace/>congtacvien' id="congtacvien"  value="<%=objNewsPDF.getCongtacvien() %>" type="text" class="form-control"></input>											
										</div>
										</td>	
									</tr>
									<tr colspan="100">
										...
										<div class="twocolumn1">
									     ...
									     <%
											...
											if(role.getRole_edit() == 1 || role.getRole_public() == 1){
												if(objNewsPDF.getIsformbinhluan() == 1){
													isformbinhluan_checked = "checked";															
												}
												if(objNewsPDF.getIsviewbinhluan() == 1){
									...
									%>			
												<aui:button type="submit" cssClass="btn-pm btn-submit-pm" name="luu" value="Lưu bài viết" label=""></aui:button>
												<%if(objNewsPDF.getId() != 0){
														if(objNewsPDF.getStatus() == 1){
												%>
