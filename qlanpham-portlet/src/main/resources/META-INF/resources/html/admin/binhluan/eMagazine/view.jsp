<%@page import="qlanphamdb.service.eMagazineLocalServiceUtil"%>
<%@page import="qlanphamdb.model.eMagazine"%>
<%@page import="qlanphamdb.model.eMagazine_BinhLuan"%>
...
<%
    ...
    int end = start + delta;
    List<eMagazine_BinhLuan> binhLuanList = QlanphamAdminUtil.listBinhLuanMagazine(noiDung, ngaybatdau, ngayketthuc, trangThai, start, end);
 	int total = QlanphamAdminUtil.listBinhLuanMagazine(noiDung, ngaybatdau, ngayketthuc, trangThai, 0, 0).size();
		...
		<div class="card-body">
			...
			<table class="table table-bordered table-hover" id="thongkeluotxem">
				...
				<tbody>
					<%
						int i = 0;
						for (eMagazine_BinhLuan bl : binhLuanList) {
							i++;
					%>
					<tr>
						<td><input name="<portlet:namespace/>check[]" class="checkBoxClass" type="checkbox" value="<%=bl.getId()%>"></input></td>
				      	...
				      	<td><%=bl.getHoTen() %></td>
				      	<%
				      		eMagazine magazine = eMagazineLocalServiceUtil.geteMagazine(bl.getMagazineId());
				      	%>
				      	<td><a target="_blank" href="<%="/chi-tiet-emagazine?dinhdanh=" + magazine.getId() %>"><%=magazine.getTen() %></a></td>
					    ...
					    %>
				      	...
				      	<portlet:actionURL var="publishSingleBinhLuanURL" name="publishBinhLuan">
				      		<portlet:param name="magazineId" value="<%= String.valueOf(bl.getId()) %>" />
				      	</portlet:actionURL>
