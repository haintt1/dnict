...
<portlet:renderURL var="addURL">
	<portlet:param name="jspPage" value="/html/admin/tapchidulich/add.jsp" />
	<portlet:param name="itemSelectorURL" value="<%= itemSelectorURL %>" />
</portlet:renderURL>
<portlet:renderURL var="softDeleteURL">
	<portlet:param name="jspPage" value="/html/admin/tapchidulich/trash.jsp" />
</portlet:renderURL>
	...
	<div class="card">
		<div class="card-body">
					...
					<div class="col-md-6">
						<aui:select name="anPham" label="Ấn phẩm" id="selectLoai">
							...
							<%
								...
								%>
							<aui:option value="<%=listLoai.getId()%>" selected="<%=listLoai.getOrders() == 2 %>"><%=listLoai.getTen()%></aui:option>
							<% } %>
						</aui:select>
					</div>
					<div class="col-md-6">
			...
			<div class="new-container-pm ">
				<portlet:renderURL var="editTapChiDuLichURL">
					<portlet:param name="id" value="<%=String.valueOf(listTapChi.get(a).getId())%>" />
					<portlet:param name="jspPage" value="/html/admin/tapchidulich/edit.jsp" />
					<portlet:param name="itemSelectorURL" value="<%= itemSelectorURL %>" />
				</portlet:renderURL>
				<portlet:renderURL var="xemPhienBanURL">
					<portlet:param name="id" value="<%=String.valueOf(listTapChi.get(a).getId())%>" />
					<portlet:param name="jspPage" value="/html/admin/tapchidulich/xemphienban.jsp" />
				</portlet:renderURL>
				<portlet:actionURL name="deleteTapChiDuLich" var="deleteTapChiDuLichURL">
					<portlet:param name="id" value="<%=String.valueOf(listTapChi.get(a).getId())%>" />
					<portlet:param name="jspPage" value="/html/admin/tapchidulich/view.jsp" />
				</portlet:actionURL>
				<div class="dropdown">
					<button onclick="myFunction(<%=listTapChi.get(a).getId()%>)" class="dropbtn"><i class="caret"></i></button>
					<div id="myDropdown_<%=listTapChi.get(a).getId()%>"
						...
						<%
							...
							if(listTapChi.get(a).getLoaiTin()==0){
								chonLoaiTin = editTapChiDuLichURL;
								deleteTin = deleteTapChiDuLichURL.toString();
							} else {
								chonLoaiTin = editTapChiDuLichURL;
								deleteTin = deleteTapChiDuLichURL.toString();
							}
