...
public class dinhkemfilePersistenceImpl
	 ...
	 */
	@Override
	public List<dinhkemfile> findByObjectId(long ObjectId) {
	 ...
	 */
	@Override
	public List<dinhkemfile> findByObjectId(long ObjectId, int start, int end) {
	 ...
	 */
	@Override
	public List<dinhkemfile> findByObjectId(
	...
	public List<dinhkemfile> findByObjectId(
		...
		if (useFinderCache) {
			...
			if ((list != null) && !list.isEmpty()) {
				for (dinhkemfile dinhkemfile : list) {
					if (ObjectId != dinhkemfile.getObjectId()) {
						list = null;
		...
		if (list == null) {
			...
			sb.append(_SQL_SELECT_DINHKEMFILE_WHERE);
			...
			if (orderByComparator != null) {
				...
				sb.append(dinhkemfileModelImpl.ORDER_BY_JPQL);
			}
	...
	public dinhkemfile findByObjectId_First(
		...
		dinhkemfile dinhkemfile = fetchByObjectId_First(
		...
		throw new NoSuchdinhkemfileException(sb.toString());
	}
	...
	public dinhkemfile fetchByObjectId_First(
		...
		List<dinhkemfile> list = findByObjectId(
	...
	public dinhkemfile findByObjectId_Last(
		...
		dinhkemfile dinhkemfile = fetchByObjectId_Last(
			ObjectId, orderByComparator);
	...
	public dinhkemfile[] findByObjectId_PrevAndNext(
		...
		dinhkemfile dinhkemfile = findByPrimaryKey(id);
		...
		try {
			...
			dinhkemfile[] array = new dinhkemfileImpl[3];

			array[0] = getByObjectId_PrevAndNext(
				session, dinhkemfile, ObjectId, orderByComparator, true);

			array[1] = dinhkemfile;

			array[2] = getByObjectId_PrevAndNext(
				session, dinhkemfile, ObjectId, orderByComparator, false);
	...
	protected dinhkemfile getByObjectId_PrevAndNext(
		Session session, dinhkemfile dinhkemfile, long ObjectId,
		OrderByComparator<dinhkemfile> orderByComparator, boolean previous) {
		...
		if (orderByComparator != null) {
			for (Object orderByConditionValue :
					orderByComparator.getOrderByConditionValues(dinhkemfile)) {

				queryPos.add(orderByConditionValue);
			}
		}

		List<dinhkemfile> list = query.list();
	...
	public int countByObjectId(long ObjectId) {
		...
		if (count == null) {
			...
			sb.append(_SQL_COUNT_DINHKEMFILE_WHERE);
	...
	public dinhkemfile updateImpl(dinhkemfile dinhkemfile) {
		...
		if (!(dinhkemfile instanceof dinhkemfileModelImpl)) {
			InvocationHandler invocationHandler = null;

			if (ProxyUtil.isProxyClass(dinhkemfile.getClass())) {
				invocationHandler = ProxyUtil.getInvocationHandler(dinhkemfile);

				throw new IllegalArgumentException(
					"Implement ModelWrapper in dinhkemfile proxy " +
						invocationHandler.getClass());
			}

			throw new IllegalArgumentException(
				"Implement ModelWrapper in custom dinhkemfile implementation " +
					dinhkemfile.getClass());
		}

		dinhkemfileModelImpl dinhkemfileModelImpl =
			(dinhkemfileModelImpl)dinhkemfile;
		...
		entityCache.putResult(
			dinhkemfileImpl.class, dinhkemfileModelImpl, false, true);
	...
	private static final String _SQL_SELECT_DINHKEMFILE_WHERE =
		"SELECT dinhkemfile FROM dinhkemfile dinhkemfile WHERE ";

	private static final String _SQL_COUNT_DINHKEMFILE =
		"SELECT COUNT(dinhkemfile) FROM dinhkemfile dinhkemfile";

	private static final String _SQL_COUNT_DINHKEMFILE_WHERE =
		"SELECT COUNT(dinhkemfile) FROM dinhkemfile dinhkemfile WHERE ";
	...
	private static final String _NO_SUCH_ENTITY_WITH_KEY =
		"No dinhkemfile exists with the key {";
