...
<%
String tabCoquanbanhanh = VanbanphapquyAdminField.value_tabCoQuanBanHanh;
String name = "";
int status= -1;
...
actionURL.setParameter("tabs", tabCoquanbanhanh);
actionURL.setParameter("name", name);
actionURL.setParameter("status", String.valueOf(status));

PortletURL searchURL = renderResponse.createRenderURL();
searchURL.setParameter("tabs", tabCoquanbanhanh);



PortletURL iteratorUrl = renderResponse.createRenderURL();
iteratorUrl.setParameter("jspPage", "/html/vanbanphapquyadmin/coquanbanhanh/list.jsp");
...
<fieldset class="fs-admin">
	...
	<table class="tbcoreadmin">
		<tr>
			...
			<a class="float-btn-input" href="${ addCoquanbanhanhURL}"><aui:button value="Thêm cơ quan ban hành"/></a>
			</td>
		</tr>
	</table>
	</form>
...
<table>
<liferay-ui:search-container delta="20" total="<%= VanbanphapquyAdminUtil.countCoquanbanhanhByKeyword(renderRequest, name, status, 0, 0).size() %>" 
	emptyResultsMessage="Chưa có liên kết nào" 
	iteratorURL="<%=actionURL%>">
	<liferay-ui:search-container-results
		results="<%= VanbanphapquyAdminUtil.countCoquanbanhanhByKeyword(renderRequest, name, status, searchContainer1.getStart(), searchContainer1.getEnd())%>"
	/>

	<liferay-ui:search-container-row
		className="vn.dnict.vanbanphapquy.model.Coquanbanhanh"
		modelVar="varCoquanbanhanh"
		keyProperty="id"
	>
		<liferay-ui:search-container-column-text name="STT" align="center"><%=stt++ %></liferay-ui:search-container-column-text>
		<liferay-ui:search-container-column-text name="Tên cơ quan ban hành" property="name"/>
		<liferay-ui:search-container-column-text name="Mã cơ quan ban hành" property="code"/>
		<%
			Coquanbanhanh cqbh = CoquanbanhanhLocalServiceUtil.fetchCoquanbanhanh(varCoquanbanhanh.getId());
			String trangthai = "";
			if(varCoquanbanhanh.getStatus()== 0){
		...
		<liferay-ui:search-container-column-jsp path="/html/vanbanphapquyadmin/coquanbanhanh/actions.jsp" name="Thao tác" align="center" cssClass="thaotac"/>
	</liferay-ui:search-container-row>
