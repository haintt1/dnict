...
actionURL.setParameter("tabs", tabLinhvucvanban);
actionURL.setParameter("name", name);
actionURL.setParameter("status", String.valueOf(status));

PortletURL searchURL = renderResponse.createRenderURL();
searchURL.setParameter("tabs", tabLinhvucvanban);



PortletURL iteratorUrl = renderResponse.createRenderURL();
iteratorUrl.setParameter("jspPage", "/html/vanbanphapquyadmin/linhvuc/list.jsp");
...
<fieldset class="fs-admin">
	...
	<table class="tbcoreadmin">
		<tr>
			...
			<a class="float-btn-input" href="${ addLinhvucvanbanURL}"><aui:button value="Thêm lĩnh vực"/></a>
			</td>
		</tr>
	</table>
	</form>
...
<table>
<liferay-ui:search-container delta="20" total="<%= VanbanphapquyAdminUtil.countLinhvucByKeyword(renderRequest, name, status, 0, 0).size() %>" 
	emptyResultsMessage="Chưa có liên kết nào" 
	iteratorURL="<%=actionURL%>">
	<liferay-ui:search-container-results
		results="<%= VanbanphapquyAdminUtil.countLinhvucByKeyword(renderRequest, name, status, searchContainer1.getStart(), searchContainer1.getEnd())%>"
	/>

	<liferay-ui:search-container-row
		className="vn.dnict.vanbanphapquy.model.Linhvucvanban"
		modelVar="varLinhvucvanban"
		keyProperty="id"
	>
		<liferay-ui:search-container-column-text name="STT" align="center"><%=stt++ %></liferay-ui:search-container-column-text>
		<liferay-ui:search-container-column-text name="Tên lĩnh vực văn bản" property="name"/>
		<liferay-ui:search-container-column-text name="Mã lĩnh vực văn bản" property="code"/>
		<%
			Linhvucvanban lvvb = LinhvucvanbanLocalServiceUtil.fetchLinhvucvanban(varLinhvucvanban.getId());
			String trangthai = "";
			if(varLinhvucvanban.getStatus()== 0){
		...
		<liferay-ui:search-container-column-jsp path="/html/vanbanphapquyadmin/linhvuc/actions.jsp" name="Thao tác" align="center" cssClass="thaotac"/>
	</liferay-ui:search-container-row>
