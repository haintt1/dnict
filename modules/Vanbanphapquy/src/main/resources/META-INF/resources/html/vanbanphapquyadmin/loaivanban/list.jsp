...
actionURL.setParameter("tabs", tabLoaivanban);
actionURL.setParameter("name", name);
actionURL.setParameter("status", String.valueOf(status));

PortletURL searchURL = renderResponse.createRenderURL();
searchURL.setParameter("tabs", tabLoaivanban);



PortletURL iteratorUrl = renderResponse.createRenderURL();
iteratorUrl.setParameter("jspPage", "/html/vanbanphapquyadmin/loaivanban/list.jsp");
...
<fieldset class="fs-admin">
	...
	<table class="tbcoreadmin">
		<tr>
			...
			<a class="float-btn-input" href="${ addLoaivanbanURL}"><aui:button value="Thêm loại văn bản"/></a>
			</td>
		</tr>
	</table>
	</form>
...
<table>
<liferay-ui:search-container delta="20" total="<%= VanbanphapquyAdminUtil.countLoaivanbanByKeyword(renderRequest, name, status, 0, 0).size() %>" 
	emptyResultsMessage="Chưa có liên kết nào" 
	iteratorURL="<%=actionURL%>">
	<liferay-ui:search-container-results
		results="<%= VanbanphapquyAdminUtil.countLoaivanbanByKeyword(renderRequest, name, status, searchContainer1.getStart(), searchContainer1.getEnd())%>"
	/>

	<liferay-ui:search-container-row
		className="vn.dnict.vanbanphapquy.model.Loaivanban"
		modelVar="varLoaivanban"
		keyProperty="id"
	>
		<liferay-ui:search-container-column-text name="STT" align="center"><%=stt++ %></liferay-ui:search-container-column-text>
		<liferay-ui:search-container-column-text name="Tên loại văn bản" property="name"/>
		<liferay-ui:search-container-column-text name="Mã loại văn bản" property="code"/>
		<%
			Loaivanban lvb = LoaivanbanLocalServiceUtil.fetchLoaivanban(varLoaivanban.getId());
			String trangthai = "";
			if(varLoaivanban.getStatus()== 0){
		...
		<liferay-ui:search-container-column-jsp path="/html/vanbanphapquyadmin/loaivanban/actions.jsp" name="Thao tác" align="center" cssClass="thaotac"/>
	</liferay-ui:search-container-row>
