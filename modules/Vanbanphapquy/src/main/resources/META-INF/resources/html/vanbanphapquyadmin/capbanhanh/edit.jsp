...
<%
	...
	Capbanhanh editCapbanhanh = (Capbanhanh) request.getAttribute("editCapbanhanh");
%>
<portlet:actionURL var="updateCapbanhanhURL" name="updateCapbanhanh">
	<portlet:param name="tabs" value="<%=tabCapbanhanh %>"/>
</portlet:actionURL>
					...
					<fieldset aria-labelledby="Title" class=" " role="group">
						<div aria-labelledby="Header" class="in  " id="Content" role="presentation">
							<div class="panel-body">
								<aui:form action="<%=updateCapbanhanhURL.toString() %>" name="" method="POST">
									<table class="tbcoreadmin">
										<tr>
											<td class="key label2">Tên</td>
											<td>
												<aui:input name="id" type="hidden" value="<%=editCapbanhanh.getId() %>"/>
												<aui:input name="name" size="40" label="" value="<%=editCapbanhanh.getName() %>">
										...
										<tr>
											...
											<td>
												<aui:input name="code" size="40" label="" value="<%=editCapbanhanh.getCode() %>">
													<aui:validator name="required"></aui:validator>
												</aui:input>
											</td>
										</tr>
										<tr>
											...
											<td><aui:input name="status" checked="<%=editCapbanhanh.getStatus()==1?true:false%>" type="checkbox" label="Hiển thị"/></td>
										</tr>
										<tr>
											<td class="key label2">
