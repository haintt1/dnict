...
public class DocumentUpload extends MVCPortlet {
	...
	private static String ROOT_FOLDER_NAME = "CmsDnict_Upload";
	private static String ROOT_FOLDER_DESCRIPTION = "This folder is create for Vanbanphapquy documents";
...
//						String sourceFileName = uploadRequest.getFileName("uploadedFile");
	...
	public static List<dinhkemfile> getLinkdinhkembyIdAndObject(long id, long Object, ThemeDisplay themeDisplay) throws Exception {
		DynamicQuery query = dinhkemfileLocalServiceUtil.dynamicQuery();
		...
		List<dinhkemfile> listFiledinhkem = dinhkemfileLocalServiceUtil.dynamicQuery(query);
		
		if(Validator.isNull(listFiledinhkem) || listFiledinhkem.size() <= 0){
			listFiledinhkem = new ArrayList<dinhkemfile>();
		}
