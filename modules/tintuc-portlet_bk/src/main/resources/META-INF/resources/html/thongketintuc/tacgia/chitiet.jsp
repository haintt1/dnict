...
if(Validator.isNotNull(ParamUtil.getString(request, "nguontin"))){
	if(Validator.isNumber(ParamUtil.getString(request, "nguontin"))){
		nguontin = ParamUtil.getLong(request, "nguontin");
	}
}
//System.out.print("ten" + nguontin);
...
//pagingUrl.setParameter("nguontin", String.valueOf(nguontin));
pagingUrl.setParameter("jspPage", "/html/thongketintuc/tacgia/chitiet.jsp");
pagingUrl.setParameter("tabs", tabtknguontin);

PortletURL searchURL = renderResponse.createRenderURL();
searchURL.setParameter("jspPage", "/html/thongketintuc/tacgia/chitiet.jsp");
searchURL.setParameter("nguontin",String.valueOf(nguontin));
pagingUrl.setParameter("tabs", tabtknguontin);
...
List<News_Article> listArticle = ThongKeTinTucUtil.thongKeChiTietNguonTin(Long.valueOf(nguontin), Integer.valueOf(status) , dateBegin, dateEnd, 0, 0);
total = ThongKeTinTucUtil.thongKeChiTietNguonTin(Long.valueOf(nguontin), Integer.valueOf(status) , dateBegin, dateEnd, 0, 0).size();
searchContainer.setTotal(total);
searchContainer.setResults(listArticle);
//News_Type newtype = News_TypeLocalServiceUtil.fetchNews_Type(nguontin);
News_NguonTin newNguonTin = News_NguonTinLocalServiceUtil.fetchNews_NguonTin(nguontin);
%>
...
<h4 style="margin-top:10px">Thống kê bài viết của 1 nguồn tin</h4>
<form  action="<%=searchURL.toString()%>" method="post" id="frmthongke" name="frmthongke">
	<table class="table table-bordered">
		...
		<tr>					
			<td class="key" valign="top" style="text-align: center;" colspan="6">
				...
				<a class="btn btn-success btn-small" href="#" id="btnReportthongketheoNguonTin">Xuất Excel</a>
	...
	<table class="table table-bordered table-list" id="resultsTable">
		...
		<tr>
			<th colspan="6">
				<span style="text-transform: uppercase;">TỔNG CỘNG <span style="color: #cc1314"><%=total%></span> BÀI VIẾT THUỘC NGUỒN TIN <span style="color: #cc1314"><%=newNguonTin.getName() %></span></span>
			</th>
		</tr>
		<%
		int stt =1;
...
$j(document).ready(function(){
	...
	$j("#btnReportthongketheoNguonTin").click(function(){
		$j("#resultsTable").table2excel({
			name: "ThongKeChiTietNguonTin",
			filename: "ThongKeChiTietNguonTin",
			fileext: ".xls",
			exclude_img: true,
			exclude_links: true,
			exclude_inputs: true
