...
%>
<portlet:actionURL name="updateBooks" var="updateBooksURL">
	<%-- <portlet:param name="contents" value="<%= String.valueOf(objNews.getContent()) %>" /> --%>
</portlet:actionURL>
				...
				<div aria-multiselectable="true" class="panel-group" role="tablist">
					<fieldset aria-labelledby="Title" class=" " role="group">
						<div aria-labelledby="Header" class="in  " id="Content" role="presentation">
							...
							<aui:form name="frmAddNews" action="<%=updateBooksURL.toString()%>" method="post" enctype="multipart/form-data">
								...
								<%	
										List<News_AttachFile> listImage = DocumentUpload.getLinkdinhkembyIdAndObject(objNews.getId(), 9);
										...
										if(listImage.size() > 0){
										 	for(int i = 0; i< listImage.size(); i++){
										 		...
										 		//System.out.print(arStr.length);
										 		//System.out.print(arStr[0]);
										 		//System.out.println("url: " +listDinhkem.get(i).getUrl());
...
$('#btndelete').click(function(){
	...
	AUI().use('aui-io-request','aui-loading-mask-deprecated', function(A){
		...
		loadingMask.show();	
		alert("Xóa ảnh đại diện của sách. Hệ thống sẽ hiển thị thông báo nếu xóa thành công");
		A.io.request(
	    url,
	      {
	        on: {  
