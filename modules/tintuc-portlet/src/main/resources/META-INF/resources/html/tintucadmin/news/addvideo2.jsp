...
%>
<portlet:actionURL name="saveNewsArticleVideo" var="saveNewsArticleVideoURL">
</portlet:actionURL>
				...
				<div aria-multiselectable="true" class="panel-group" role="tablist">
					<fieldset aria-labelledby="Title" class=" " role="group">
						<div aria-labelledby="Header" class="in  " id="Content" role="presentation">
							...
							<aui:form name="frmAddNewsVideo" action="<%=saveNewsArticleVideoURL.toString() %>"method="post" enctype="multipart/form-data" autocomplete="off">
									...
									<tr>
										<td style="padding-top: 10px">
											<div class="twocolumn1">
												<input name="option" type="radio" value="uploadFile" onchange="toggleInput('upload')" checked="checked"> 
...
function ValidateSingleInputVideo(oInput) {
	const _validFileExtensionsMP4 = ["mp4"];
    if (oInput.type == "file") {
        var sFileName = oInput.value;
         if (sFileName.length > 0) {
        	 const extensionMP4 = sFileName.substring(sFileName.lastIndexOf('.') + 1).toLowerCase()
        	 let blnValids = _validFileExtensionsMP4.includes(extensionMP4)            
        	 if (!blnValids) {
                 alert("Xin lỗi, " + sFileName + " bị lỗi, tập tin phải là : " + _validFileExtensionsMP4);
                 oInput.value = "";
                 return false;
             }
        }
...
AUI().ready('aui-form-validator', 'aui-overlay-context-panel','aui-node', function(A) {
	var validator2 = new A.FormValidator({
	        boundingBox: document.<portlet:namespace/>frmAddNewsVideo,
	        on: {
...
<aui:script>
    ...
    function toggleInput(option) {
        ...
        if (option === 'link') {
            formInput.innerHTML = `
                <aui:input name="linkVideo" label="Nhập đường dẫn" type="text"></aui:input>
            `;
