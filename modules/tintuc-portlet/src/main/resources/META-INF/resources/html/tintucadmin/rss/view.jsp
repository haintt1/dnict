...
<%@page import="vn.dnict.tintuc.utils.RssUtil"%>
...
<portlet:renderURL var="viewRss">
	<portlet:param name="jspPage" value="/html/tintucadmin/rss/rss.jsp" />
</portlet:renderURL>
<%-- <portlet:renderURL var="configRss">
	<portlet:param name="jspPage" value="/html/tintucadmin/rss/configDanhmuc.jsp" />
</portlet:renderURL> --%>
	...
	<div class="content_block">
		...
		<%
			if(Validator.isNotNull(multicategory)){
				...
				for(int i=0; i<num;i++){
					...
					String cateName = RssUtil.removeAccent(category.getName().replace(" ", "_").toLowerCase().toString()) + "-rss.xml";
					String rssUrl = "#"; 
		...
		<div class="list_item_rss list_left">
         ...
         <%}}else{
 			...
 			num=listCate.size()/2;
 			for(int i=0; i<num;i++){
 				String cateName = RssUtil.removeAccent(listCate.get(i).getName().replace(" ", "_").toLowerCase().toString()) + "-rss.xml";
		...
		%>
         ...
         <%}}else{
 			...
 			for(num2=listCate2.size()/2; num2<listCate2.size();num2++){
 				String cateName = RssUtil.removeAccent(listCate2.get(num2).getName().replace(" ", "_").toLowerCase().toString()) + "-rss.xml";
 				String rssUrl = "#"; 
 				rssUrl = PortalUtil.getPortalURL(request) + "/rss/" +  cateName;
 				//System.out.println(rssUrl);
