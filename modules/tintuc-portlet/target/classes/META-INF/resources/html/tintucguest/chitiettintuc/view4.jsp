				...
				<ul class="root">
					...
					<li class="last-li">
						<%
							...
							if(Validator.isNotNull(cat_breakcrump) && !cat_breakcrump.equals("")){
								if(Validator.isNumber(cat_breakcrump) && cat_breakcrump.length()>0  &&  Long.valueOf(cat_breakcrump) > 0){
									ncat = News_CategoriesLocalServiceUtil.getNews_Categories(Long.valueOf(cat_breakcrump));
								}
								News_Categories newcat = null;
								if(ncat == null || ncat.getId()==0){
									ncat =  newcat;
								}
							}
							String urlcat = trangchitiet+"?danhmuc="+ncat.getId();
						%>
						<a href="<%=urlcat%>"><%=ncat.getName()%></a>
					</li>
				</ul>
			</div>
			<%}%>
					...
					<div id="textToSpeech" >					
						...
						<div class="the-article-body grid"  style="margin-top: 0px">
							...
							if(new_article.getLoaitintuc() == 2 && kieuhienthipdf.equals("lattrang")){
								...
								noidungchitiet = noidungchitiet + "<a style=\"display: block;\" class=\"xemfull-pdf\" target=\"_BLANK\" href=\"/TinTuc-portlet/js/pdf-flipbook-master/index.html?file="+urlPdf+"\">Xem file ở chế độ toàn màn hình</a>";
								noidungchitiet = noidungchitiet + "<p class=\"pdfview\" style=\"text-align:center\"><embed src=\"/TinTuc-portlet/js/pdf-flipbook-master/index.html?file="+urlPdf+"\" style=\"width:80%; height:1000px;\"></embed></p>";
...
<!-- 							</a> -->
					...
					<ul class="cat-lienquan">
						<%for(int g = 0; g< listarticle2Category.size(); g++){%>
							<li>
								<%
									News_Categories newcat = News_CategoriesLocalServiceUtil.fetchNews_Categories(listarticle2Category.get(g).getNewcategoryId());
									String urlcat = trangchitiet+"?danhmuc="+newcat.getId();
								%>
								<a href="<%=urlcat%>"><%=newcat.getName()%></a>
							</li>
						<%}%>
					</ul>
					<%}%>
						...
						<ul>
							<%
								...
								for(int i=0;i<more_article.size();i++){
									...
									News_Categories	ncat = News_CategoriesLocalServiceUtil.getNews_Categories(Long.valueOf(cat_breakcrump));	
			...
			(function(d, s, id) {
			    ...
			    js = d.createElement(s); js.id = id;
			   	js.src = "//connect.facebook.net/en_US/sdk.js#xfbml=1&version=v3.0&appId=131102320706904";
...
<%}else if(Validator.isNotNull(idcat) && Validator.isNumber(idcat) && Long.valueOf(idcat) > 0){%>
		...
		<%
			...
			int total 		 = TintucGuestUtil.listTintuc("", "", idcat, 0, 0).size();
			...
			listnew_article  = TintucGuestUtil.listTintuc("", "", idcat, start, end);
			...
			%>
				<div class="item-list-article">
					...
					<div class="item-list-article-content">
						...
						<%
// 							String noidung = listnew_article.get(b).getContent();
