...
<portlet:actionURL name="saveCategory" var="saveCategoryURL">
	<portlet:param name="languages" value="<%=ParamUtil.getString(request, "btnlang") %>"/>
				...
				<div aria-multiselectable="true" class="panel-group" role="tablist">
					<fieldset aria-labelledby="Title" class=" " role="group">
						<div aria-labelledby="Header" class="in  " id="Content" role="presentation">
							...
							<aui:form name="formAddThongTin" action="<%=saveCategoryURL.toString() %>"
								method="post">
								<table class="tbcoreadmin" style="width: 100%">
									<tr>
										<td colspan="100">
											<label class="control-label">Name</label>
											<aui:input name="name" label="" type="input">
									...
									<tr>
										<td>
											...
											<select class="form-control"  id="languages" name='<portlet:namespace/>languages'>
									     		<option value="vi_VN" selected="selected">Tiếng việt (Mặc định)</option>
									     		<option value="en_US">Tiếng Anh</option>
									  <!-- 		<option value="ja_JP">Tiếng Nhật</option>
									     		<option value="ko_KR">Tiếng Hàn</option> -->
									     	</select>
									 ...
									 <tr>
										<td>
											...
											<aui:option value="" selected="true">Chọn thư mục chứa</aui:option>
											<%
												List<News_Subcategories> res = TinTucAdminUtil.countNews_SubCategories(renderRequest, "", 0, 0);
											...
											%>
											<aui:option value="<%=res.get(i).getId()%>">- <%=res.get(i).getName()%></aui:option>
											<%}%>
											</aui:select>
										</td>	
									</tr>
