...
<div class="containerlibcat">
		...
		<%
			long idsubcat = 1;
			List<News_Categories> listcategory = TinTucAdminUtil.getNews_CategoriesByIdSubCatLang(renderRequest,checkLangNews,idsubcat,"",0, 0);
			if(listcategory.size() > 0){
		%>
			<ul class="danhmuc">
				<%
				List<News_Article2Category> listart2Cat = TinTucAdminUtil.getListArticle2CategoryByArticleId(Long.valueOf(id));
				for(int a = 0; a < listcategory.size(); a++){
					String selected = "";
					for(int d = 0; d < listart2Cat.size(); d++){
						if(listart2Cat.get(d).getNewcategoryId() == listcategory.get(a).getId()){
							selected = "checked";
						}
					}
					if(category_id != null){
